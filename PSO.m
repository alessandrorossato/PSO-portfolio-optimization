...
format long;
%% Load data
data = readtable('Data/2015-04-01_2020-04-01.csv');
...
num_portfolios = 100; % Number of portfolios on the efficient frontier
target_returns = linspace(max(min(mean_returns),0), max(mean_returns), num_portfolios);
...
risk_aversion_values = linspace(0, 1, 30); % Varying lambda values
...
for lambda = 1:numel(risk_aversion_values)
    ...
    for iter = 1:num_iterations
        ...
        for i = 1:num_particles
            ...
            fitness = compute_fitness(particle_positions(i, :), risk_aversion, mean_returns, cov_matrix, num_selected_assets, epsilon, delta, max_returns, max_variance, mean_returns_val, mean_variance);
        ...
        if abs(prev_best_score - global_best_score) < 1e-8 % If no improvement in score
        ...
        if num_iterations_without_improvement >= 50 % Terminate if no improvement for 100 iterations
            % Check the global_best_score 
            if (global_best_score > 1000) % && num_restart < 100
...
%% Basic PSO
% Reusing variables with the prefix basic_
basic_PSO_portfolios = zeros(numel(risk_aversion_values), 3 + num_assets);
...
for lambda = 1:numel(risk_aversion_values)
    ...
    % Initialize particle positions and velocities within boundary constraints
    basic_particle_positions = rand(num_particles, num_assets);
    basic_particle_positions = lower_bounds + basic_particle_positions .* (upper_bounds - basic_particle_positions);
    basic_particle_velocities = zeros(num_particles, num_assets);
    
    % Binary selection variables
    basic_binary_positions = zeros(num_particles, num_assets);
    
    for i = 1:num_particles
        selected_indices = randperm(num_assets, num_selected_assets);
        basic_binary_positions(i, selected_indices) = 1;
    end
    
    basic_personal_best_positions = basic_particle_positions;
    basic_personal_best_scores = inf(num_particles, 1);
    basic_global_best_position = zeros(1, num_assets);
    basic_global_best_score = inf;

    % Initialize termination criterion
    basic_num_iterations_without_improvement = 0;
    basic_prev_best_score = inf;
    ...
    for iter = 1:num_iterations
        ...
        for i = 1:num_particles
            % Update velocities
            basic_particle_velocities(i, :) = w * basic_particle_velocities(i, :) + ...
                c1 * rand * (basic_personal_best_positions(i, :) - basic_particle_positions(i, :)) + ...
                c2 * rand * (basic_global_best_position - basic_particle_positions(i, :));
            
            % Update positions
            basic_particle_positions(i, :) = basic_particle_positions(i, :) + basic_particle_velocities(i, :);
    
            % Update positions within the cardinality constraint
            current_set = find(basic_binary_positions(i, :));
            Knew = length(current_set);
            ...
            while Knew < num_selected_assets
                available_indices = setdiff(1:num_assets, current_set);
                new_asset = available_indices(randi(length(available_indices)));
                basic_binary_positions(i, new_asset) = 1;
                basic_particle_positions(i, new_asset) = epsilon; % Assign minimum proportional value
                current_set = find(basic_binary_positions(i, :));
                Knew = length(current_set);
            end
            ...
            % Removing Assets: if Knew > num_selected_assets
            while Knew > num_selected_assets
                [~, sorted_indices] = sort(basic_particle_positions(i, current_set));
                asset_to_remove = current_set(sorted_indices(1));
                basic_binary_positions(i, asset_to_remove) = 0;
                basic_particle_positions(i, asset_to_remove) = 0;
                current_set = find(basic_binary_positions(i, :));
                Knew = length(current_set);
            end
            ...
            % Proportional Value Adjustment
            for j = current_set
                if basic_particle_positions(i, j) < epsilon
                    basic_particle_positions(i, j) = epsilon;
                elseif basic_particle_positions(i, j) > delta
                    basic_particle_positions(i, j) = delta;
                end
            end
    
            total_weight = sum(basic_particle_positions(i, current_set));
            if total_weight > 0
                basic_particle_positions(i, current_set) = basic_particle_positions(i, current_set) / total_weight;
            end
            
            % Evaluate fitness
            %fitness = fitness_function(basic_particle_positions(i, :), risk_aversion);
            fitness = compute_fitness(basic_particle_positions(i, :), risk_aversion, mean_returns, cov_matrix, num_selected_assets, epsilon, delta, max_returns, max_variance, mean_returns_val, mean_variance);

            % Update personal bests
            if fitness < basic_personal_best_scores(i)
                basic_personal_best_scores(i) = fitness;
                basic_personal_best_positions(i, :) = basic_particle_positions(i, :);
            end

            % Update global best
            if fitness < basic_global_best_score
                basic_global_best_score = fitness;
                basic_global_best_position = basic_particle_positions(i, :);
            end
        end
        
        % Check termination criterion
        if abs(basic_prev_best_score - basic_global_best_score) < 1e-10 % If no improvement in score
            num_iterations_without_improvement = num_iterations_without_improvement + 1;
        else
            num_iterations_without_improvement = 0; % Reset counter
            basic_prev_best_score = basic_global_best_score;
        end

        if num_iterations_without_improvement >= 500 % Terminate if no improvement for 100 iterations
            % Check the global_best_score 
            if (basic_global_best_score > 1000) && num_restart < 5
                fprintf('Reinitializing parameters at iteration %d for lambda = %.2f\n', iter, risk_aversion_values(lambda));

                % Reinitialize all parameters
                basic_particle_positions = rand(num_particles, num_assets);
                basic_particle_positions = lower_bounds + basic_particle_positions .* (upper_bounds - lower_bounds);
                basic_particle_velocities = zeros(num_particles, num_assets);
                basic_binary_positions = zeros(num_particles, num_assets);
        
                for ij = 1:num_particles
                    basic_selected_indices = randperm(num_assets, num_selected_assets);
                    basic_binary_positions(ij, selected_indices) = 1;
                end
    ...
    portfolio_return = global_best_position * mean_returns;
    portfolio_variance = global_best_position * cov_matrix * global_best_position';
    basic_PSO_portfolios(lambda, :) = [risk_aversion, portfolio_variance, portfolio_return, global_best_position];
    disp('Minimum score:');
    disp(global_best_score);
end
...
% basic_PSO_variances = basic_PSO_portfolios(:, 2);
% basic_PSO_returns = basic_PSO_portfolios(:, 3);
...
% Plot Basic PSO results
scatter(basic_PSO_variances, basic_PSO_returns, 50, 'g', 'filled', 'DisplayName', 'Basic PSO Results');

xlabel('Portfolio Variance (Risk)');
ylabel('Portfolio Return');
title('Efficient Frontier, PSO and Basic PSO Results');
...
%%
% Fitness function 
function fitness = compute_fitness(weights, risk_aversion, mean_returns, cov_matrix, num_selected_assets, epsilon, delta, max_returns, max_variance, mean_returns_val, mean_variance)
    variance = weights * cov_matrix * weights';
    returns = mean_returns' * weights';


% Fitness function 
    fitness = risk_aversion * variance/max_variance - (1 - risk_aversion) * returns/max_returns;
    ...
    if nnz(weights) > num_selected_assets 
        fitness = fitness + 10000 * abs(nnz(weights)-num_selected_assets);

    end
    % Check if any weight is outside the bounds [epsilon, delta]
    if any(nnz(weights) < epsilon) || any(weights > delta) 
        fitness = fitness + 10000 * (sum(any(nnz(weights) < epsilon)) + sum(weights > delta));
    end
